...
const STATUS_INFO: Record<
  'organic' | 'newsoul',
  { label: string; flavour: string; advantages?: string[]; limitations?: string[] }
> = {
  organic: {
    label: 'Organic',
  ...
  newsoul: {
    label: 'Newsoul',
    flavour: 'Consciousness transferred to a synthetic body — power and limitation in equal measure.',
    advantages: [
  ...
  return (
    <div className="my-1 p-5 rounded-lg border border-gold/30 bg-panel/70 animate-expand-down">
      ...
      <div className={`grid gap-4 ${info.limitations ? 'sm:grid-cols-2' : ''}`}>
        {info.advantages && (
          <div>
            <p className="text-gold/50 text-[10px] tracking-widest uppercase mb-2">Advantages</p>
        ...
        {info.limitations && (
          <div>
            <p className="text-gold/50 text-[10px] tracking-widest uppercase mb-2">Limitations</p>
            <ul className="space-y-1">
        ...
        <div className="flex gap-3">
          {(['organic', 'newsoul'] as const).map((status) => {
            ...
            return (
              <button
                key={status}
                onClick={() => update({ newsoul: isSelected ? '' as NewSoulStatus : status })}
