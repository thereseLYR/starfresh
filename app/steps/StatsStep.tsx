      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {STAT_NAMES.map(stat => (
          <div key={stat} className="border border-gold/25 rounded p-4 text-center">
            <div className="text-gold/60 text-xs tracking-[0.2em] uppercase mb-3">
              {STAT_ABBR[stat]}
            </div>
            <div className="text-gold text-4xl font-bold tabular-nums mb-1">
              {data.stats[stat]}
            </div>
            <div className="text-gold/60 text-sm font-mono mb-1">
              {statModifier(data.stats[stat])}
            </div>
            <div className="text-gold/60 text-xs mb-4">{stat}</div>
