...
import { CharacterProvider, useCharacter } from '@/app/context/CharacterContext'
import { STEPS, isStepComplete } from '@/app/lib/types'

const ENFORCE_VALIDATION = process.env.NEXT_PUBLIC_ENFORCE_VALIDATION === 'true'
...
function renderStep(step: number) {
  switch (step) {
    case 0: return <CharacterInfoStep />
    case 1: return <ConstellationStep />
    case 2: return <SpeciesStep />
    case 3: return <HistoryStep />
    case 4: return <CareerStep />
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            {!isLast ? (
              <button
                ...
                className="px-5 py-2 border border-gold text-gold text-sm rounded hover:bg-gold hover:text-dark-green disabled:opacity-20 disabled:cursor-not-allowed transition-all"
              >
                Next →
              </button>
            ) : (
                  ...
                  >
                    <div
                      className={`w-7 h-7 rounded-full border-2 flex items-center justify-center text-xs font-bold transition-all ${
                        current ? 'border-gold bg-gold text-dark-green'
                        : done   ? 'border-gold/70 text-gold/70'
